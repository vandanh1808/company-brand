  ...
  useEffect(() => {
    const filtered = brands.filter(
      (brand) =>
        ...
        brand.companyId.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success("Đã xóa thương hiệu và tất cả sản phẩm liên quan");
        // Refresh header data
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-xl">Đang tải danh sách thương hiệu...</div>
        </div>
      </div>
    );
  }
              ...
              <TableBody>
                {filteredBrands.map((brand) => (
                  <TableRow key={brand._id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">{brand.companyId.name}</Badge>
                    </TableCell>
